...
sudo dnf install quickshell rsms-inter-fonts google-noto-sans-fonts adobe-source-sans-pro-fonts pipewire-devel glib2-devel kf6-kglobalaccel-devel qt6-qtbase-devel qt6-qtdeclarative-devel python3-dbus-next cmake
make requirements
```

### Scripted setup

From a cloned checkout, `./install.sh` performs the full setup on any distribution running KDE Plasma Wayland, requesting `sudo` only for system-owned paths. It verifies prerequisites — including the Inter UI font through Fontconfig and a running per-user systemd manager — and offers everything missing through the detected package manager (`rsms-inter-fonts` on Fedora, `inter-font` on Arch, `fonts-inter` on Debian; distributions without verified package mappings, such as openSUSE, receive explicit manual instructions instead of guessed package names), builds the native helpers, installs the tree under `/usr/share/nagi-shell` (`--dest` overrides this), registers the **Nagi Shell** section in KDE keyboard settings immediately, creates a private default `settings.conf` only when neither V2 settings nor the legacy migration source exists, installs/enables the ordered per-user `nagi-shell.service`, creates the desktop activation entry, and installs the `nagi-shell` launcher wrapper on `PATH`. The service is ordered before `plasma-plasmashell.service`, claims `org.freedesktop.Notifications` from session startup, and is the sole popup presenter; a fresh install never mutates the current session, an inactive upgrade remains inactive, and an active Nagi upgrade stops and restores only Nagi. Applying the new startup ordering requires the next logout/login. The launcher raises the existing singleton for Control Center activation. No XDG autostart or D-Bus activation fallback is installed.

Running the script unprivileged preserves the logged-in user's XDG roots and is the supported default. If the whole script is deliberately invoked through `sudo`, custom `XDG_CONFIG_HOME` or `XDG_STATE_HOME` values passed through sudo are honored; when sudo's environment policy strips them, the roots are recovered from the target user's systemd manager environment, with the target user's standard home-relative roots as the final fallback. In either mode, user-level paths resolve against the invoking (`SUDO_USER`) account, so the unit, legacy autostart migration, launcher detection, and uninstall agree on one location.

`./uninstall.sh --dest /usr/share/nagi-shell` reverses everything: it disables and stops `nagi-shell.service`, removes the **Nagi Shell** shortcut category through the KGlobalAccel D-Bus API (never by editing raw config files), deletes the installation, launcher, desktop and autostart entries plus `~/.config/nagi-shell` and `~/.local/state/nagi-shell`, and offers exactly one plasmashell restart — only when `org.freedesktop.Notifications` is left unowned — so Plasma can reclaim notification delivery. It intentionally keeps two bounded caches — the weather record under Quickshell's cache directory and the wallpaper thumbnail cache under `~/.cache/nagi-shell/` — because both are inert user-cache data once the shell is removed; this script never silently deletes user cache data.
...
Polkit credentials never enter the production shell because the backend is dormant. Synthetic presentation tests still enforce explicit submission and secret cleanup. The repository's stability gate enforces these boundaries mechanically: a synthetic forbidden-value corpus — Wi-Fi and hidden-network identities and secrets, Bluetooth addresses, names, and codes, notification content, weather labels and coordinates, display metadata, wallpaper paths and digests, executable and PID strings, and raw D-Bus payloads — must stay out of every private config/state/cache tree, test log, and safe diagnostic, and the cleaned post-submit captures of two different-length secrets must stay byte-identical, proving submitted secrets leave no pixel trace.
...
- One Interactive task or Modal flow exists across all islands. Notifications, Gaming Performance, and confirmed output-volume feedback may appear on every eligible island; workspace feedback appears only while every output confirms the same current desktop and is globally suppressed otherwise; brightness remains routed; sensitive flows never mirror.
...
- Audio covers backend-confirmed default output/input selection, volume, and mute. EasyEffects 8.2.8 exposes no supported preset-list API, so native preset discovery deliberately uses its current host data layout under `${XDG_DATA_HOME:-$HOME/.local/share}/easyeffects/{output,input}`. The visible Audio subview takes one bounded non-recursive snapshot on activation and Refresh, lists only regular non-symlink `.json` presets in output/input select lists, and never polls or watches the directories. Selecting an item sends one `load_preset` command and reports success only after bounded `get_last_loaded_preset` confirmation. Flatpak/private sandbox roots, unknown package layouts, arbitrary name entry, routing, autoload, process-all, bypass controls, and per-application mixing remain unsupported.
...
- Isolated virtual-KWin checks prove QML surface, focus, topology, scaling, and lifecycle contracts but cannot prove numeric GPU utilization, physical HDR/VRR, hardware frame pacing at the monitor's real refresh rate, physical hotplug, fractional client scaling, or blur-unavailable hardware behavior. These are recorded as named external gaps in `tests/stability/budgets.json`, and missing physical evidence is reported rather than obtained through host-session mutation.
...
`make check` composes `check-nondisplay` — which includes the display-independent `test-notification-startup`, `test-wallpaper`, and `test-wallpaper-dbus` — with the isolated virtual-KWin surface gates (`test-surface-state`, `test-ui-primitives`, `test-typography`, `test-control-center`, `test-display-orchestration`, `test-workspace-consensus`), the eight issue-70 platform-contract gates (`test-multi-surface`, `test-ipc-activation`, `test-appearance-watcher`, `test-settings-writer`, `test-networkmanager-contract`, `test-bluez-contract`, `test-gaming-power-contract`, `test-wallpaper-write-contract`), and `test-wallpaper-service`; `make help` lists every child gate, and the stability gate — run explicitly through `make check-stability`, which delegates to the routine `make test-stability` — plus all host-mutating or read-only live targets (`test-audio-live`, `test-audio-live-write`, `test-connectivity-live-write`, `test-brightness-live-write`, `test-wallpaper-live`, `test-global-shortcut-live`, `test-tray-live`) stay outside `make check`. `make test-notification-startup` verifies the notification ownership packaging in a hermetic sandbox: the rendered `nagi-shell.service` contract ordered after KWin and before `plasmashell` with three restarts per 60 seconds, enablement without starting, removal of legacy XDG autostart, absence of competing D-Bus activation, launcher service and checkout fallback paths, and uninstall's single plasmashell restart offer when the name is unowned. `make test-workspace-consensus` drives the production KWin workspace helper through shared, divergent, and reconverged desktop state across one, two, and three virtual outputs with private `PerOutputVirtualDesktops`, proving consensus-only publication and global suppression without a consensus. `make test-stability` chains the 21 focused soak targets — `test-control-center`, `test-weather`, `test-connectivity`, `test-connectivity-dbus`, `test-bluetooth-manager-dbus`, `test-wallpaper`, `test-wallpaper-dbus`, `test-wallpaper-service`, `test-easyeffects-status`, `test-audio`, `test-gaming-performance`, `test-notifications`, `test-media`, `test-brightness`, `test-session`, `test-applications`, `test-global-shortcut`, `test-tray`, `test-surface-state`, `test-display-orchestration`, and `test-polkit-ui` — then runs the isolated performance probe and the privacy sweep. The probe starts production `shell.qml` in the private virtual-KWin runtime for one, two, and three outputs, drives all eleven pages plus the generic `control-center` activation reason (12 reasons total) open and closed in both responsive modes across the routine 25 cycles plus appearance and blur setting changes, and samples `/proc` for 15-second windows per output count after a 20-second settle under a deterministic Mesa configuration that pins software rendering to `llvmpipe`; the renderer observed from the Qt RHI context must equal the frozen fingerprint. `make test-stability-soak` is the manual extended certification target for the same probe with 100 cycles and 30-second windows; it is not part of `make check`. Structural, count, and privacy gates are always enforced; isolated numeric ceilings apply only on an exact match with the frozen baseline fingerprint, and `NAGI_ENFORCE_PERFORMANCE=1` makes any fingerprint mismatch or missing metric a failure. The privacy sweep fails on any synthetic forbidden value outside its owning visible flow and requires the cleaned post-submit captures of two different-length secrets to be byte-identical, proving cleanup leaves no pixel trace.

Frozen budgets in `tests/stability/budgets.json`: live one-island Quickshell CPU `≤ 3.1%` of one core, RSS `≤ 283,944 KiB`, and warm startup `≤ 1.084 s`, with exactly nine persistent helpers plus the in-process notification plugin under an aggregate helper RSS ceiling of `267,346 KiB`; isolated one/two/three-island CPU `≤ 0.35/0.50/0.65%`, scheduling slices `≤ 15/18/21` per second, Quickshell RSS `≤ 524,288/786,432/983,040 KiB` (`512/768/960 MiB`), and `≤ 34` threads, with exactly the eight PipeWire-absent helpers at every output count. The request-scoped `nagi-settings` writer runs only while a write is in flight and must be absent from sampled helper sets. A closed Control Center owns zero loaded pages, page-interest work, page-owned active timers, page animations, hidden focus loops, or EasyEffects interest; idle sampling requires every helper to stay at zero CPU and scheduler activity except the event-driven `nagi-applications` helper, which may spend at most one CPU tick and twenty scheduling slices per sample; one to three displays add exactly one island surface and one wallpaper-screen entry per display with every other resource count invariant. The multi-display RSS ceilings are practical limits approved for the software-rendered probe, deliberately wider than the earlier hardware-rendering-derived extrapolation; growth and leak checks stay strict (exact count return after every cycle and settled RSS growth within the greater of 5% or 16,384 KiB). Current gate evidence lives in `build/stability/performance-*-summary.json` and the repository's local report under `docs/reports/`.
...
Keep changes scoped, event-driven, keyboard accessible, and verified with the focused harnesses and isolated virtual-KWin surface gates. Open issues and pull requests against `develop`; run the relevant targets and `make check` before submission. Before creating a release tag, run `make check-stability` and create the tag only if the gate passes.
